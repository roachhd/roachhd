...
if($_POST) {
	...
	$email_to = "youhavemail@roachd.com"; 	// <- Your Email Address
	...
	if(trim($name) == '') {
		...
		I need to know your name.</div>";
		exit();
	} else if(!filter_var($email, FILTER_VALIDATE_EMAIL)) {
		echo "<div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
		Whoopsy, that didn't look right.</div>";
		exit();
	} else if(trim($message) == '') {
		echo "<div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
		oh come on, you must have something to say o_0 </div>";
		exit();
	}
